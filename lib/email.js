...
export async function enviarAlertaAdmin(pedido) {
  const adminEmail = (process.env.OWNER_EMAIL || '').trim();
  ...
  try {
    console.log(`[Email Admin] Enviando alerta de nuevo pedido a "${adminEmail}" usando Resend...`);
    const { data, error } = await resend.emails.send({
      from: 'Panadería Sandrita <onboarding@resend.dev>',
      to: adminEmail,
    ...
    if (error) {
      console.error('[Email Admin] Resend rechazó el envío de la alerta:', error);
      return { ok: false, error: error.message || JSON.stringify(error) };
    } else {
      console.log('[Email Admin] Alerta enviada exitosamente con Resend ID:', data?.id);
      return { ok: true, data };
    }
  } catch (err) {
    console.error('[Email Admin] Excepción durante el envío de la alerta:', err);
    return { ok: false, error: err.message || err };
  }
}
